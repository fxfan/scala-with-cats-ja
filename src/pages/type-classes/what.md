...
`length` を実装できるのは、それがリストの要素である型 `A` の値に対して特に機能を必要としないからである。`A` 型の値に対してメソッドを呼び出すことはないし、そもそも、`length` が定義される時点では、`A` が具体的にどの型になるかがわからないため、メソッドを呼び出すこともできない[^abstraction]。
