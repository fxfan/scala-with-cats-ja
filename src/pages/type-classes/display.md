...
最後に、使いたい型クラスインスタンスをスコープに持ち込み、インターフェースオブジェクトもしくはインターフェース構文を用いて、型クラスを利用する。型クラスインスタンスをコンパニオンオブジェクトに定義したのであれば Scala は自動的にそれらをスコープに含めるが、そうでない場合は `import` を用いてアクセスする。
