...
[^tn-adt-interpreters-tail-recursion-01]: 【訳注】 「末尾呼び出し」は、単に「末尾位置にある関数呼び出し」を指して使われることが多いと思うが、ここでは「コンパイラやランタイムによってスタックフレームを消費しない形に最適化された関数呼び出し」のことを指しているように思われる。
